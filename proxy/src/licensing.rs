...
pub fn validate_license_detailed() -> Result<LicenseInfo> {
    ...
    // Check for API key in order of preference
    let (api_key, key_source) = if let Ok(key) = env::var("JAVELIN_API_KEY") {
        ...
        (key, "OPENAI_API_KEY (legacy)".to_string())
    } else {
        error!("No API key found in environment variables");
        return Err(ramparts_common::anyhow::anyhow!(
            "Proxy functionality requires a valid API key.\n\
            Set one of the following environment variables:\n\
            - JAVELIN_API_KEY (recommended)\n\
            - LLM_API_KEY\n\
            - OPENAI_API_KEY (legacy support)\n\n\
            To obtain an API key, visit: https://www.getjavelin.com"
        ));
    };
