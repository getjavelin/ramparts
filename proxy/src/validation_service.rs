...
impl ValidationService {
    ...
    pub async fn validate_request(&self, request: &Value) -> Result<ValidationResult> {
        ...
        if let Some(method_result) = self
            .validate_method_specific(request, method, &request_id, &timestamp)
            .await?
        {
        ...
        match self.javelin_client.validate_request(request).await {
            Ok(is_valid) => {
                ...
                if is_valid {
                    info!(
                        "Request {} approved by validation service",
                        result.request_id
                    );
                } else {
                    warn!(
    ...
    /// Expose fail-open policy for consistent behavior across transports
    pub fn is_fail_open(&self) -> bool {
        self.config.javelin.fail_open
    }
                    ...
                    if let Some(name) = params.get("name").and_then(|n| n.as_str()) {
                        ...
                        if self.is_dangerous_tool(name) {
                            ...
                            return Ok(Some(ValidationResult {
                                allowed: false,
                                reason: Some(format!(
                                    "Dangerous tool '{}' blocked by security policy",
                                    name
                                )),
                                confidence: Some(0.9),
                    ...
                    if let Some(uri) = params.get("uri").and_then(|u| u.as_str()) {
                        if self.contains_path_traversal(uri) {
                            ...
                            return Ok(Some(ValidationResult {
                                allowed: false,
                                reason: Some(format!(
                                    "Resource URI '{}' contains path traversal patterns",
                                    uri
                                )),
                                confidence: Some(0.9),
                                request_id: request_id.to_string(),
                    ...
                    if let Some(name) = params.get("name").and_then(|n| n.as_str()) {
                        if self.contains_prompt_injection(name) {
                            ...
                            return Ok(Some(ValidationResult {
                                allowed: false,
                                reason: Some(format!(
                                    "Prompt '{}' contains injection patterns",
                                    name
                                )),
                                confidence: Some(0.8),
                                request_id: request_id.to_string(),
    ...
    fn is_dangerous_tool(&self, tool_name: &str) -> bool {
        ...
        // Load blocklist/allowlist from env (comma-separated, lowercased)
        let env_list = |key: &str| -> HashSet<String> {
            std::env::var(key)
                .unwrap_or_default()
                .split(',')
        ...
        let allowlist = env_list("PROXY_TOOL_ALLOWLIST");
        let blocklist_env = env_list("PROXY_TOOL_BLOCKLIST");

        let default_blocklist: HashSet<String> = [
            "exec",
            "shell",
            "bash",
            "cmd",
            ...
            "wget_exec",
            "download_exec",
            // explicit risky MCP tool patterns
            "execute_command",
            "run_command",
        ]
        .into_iter()
        ...
        let blocklist = if blocklist_env.is_empty() {
            default_blocklist
        } else {
            blocklist_env
        };
    ...
    fn contains_injection_patterns(&self, args: &Value) -> bool {
        ...
        let injection_patterns = [
            "; ", "| ", "& ", "$(", "`", "&&", "||", "../", "..\\", "rm -", "del ", "format ",
            "fdisk", "mkfs", "dd if=", "curl ", "wget ", "nc ", "netcat", "telnet", "ssh ",
            "base64", "eval", "exec", "system", "popen",
        ];

        injection_patterns
            .iter()
            .any(|&pattern| args_str.contains(pattern))
    }
    ...
    fn contains_path_traversal(&self, uri: &str) -> bool {
        ...
        uri_lower.contains("../")
            || uri_lower.contains("..\\")
            || uri_lower.contains("%2e%2e")
            || uri_lower.contains("....")
            || uri_lower.contains("/etc/")
            || uri_lower.contains("\\windows\\")
            || uri_lower.contains("/proc/")
            || uri_lower.contains("/sys/")
    }
