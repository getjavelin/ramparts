...
impl ProxyConfig {
    ...
    pub fn from_env() -> Result<Self> {
        ...
        // Load Javelin configuration
        config.javelin.api_key = env::var("JAVELIN_API_KEY").unwrap_or_default();
        ...
        if let Ok(log_requests) = env::var("PROXY_LOG_REQUESTS") {
            config.behavior.log_requests = log_requests.parse().map_err(|e| {
                ramparts_common::anyhow::anyhow!("Invalid log_requests value: {}", e)
            })?;
        }

        if let Ok(cache) = env::var("PROXY_CACHE_VALIDATIONS") {
            config.behavior.cache_validations = cache.parse().map_err(|e| {
                ramparts_common::anyhow::anyhow!("Invalid cache_validations value: {}", e)
            })?;
        }

        if let Ok(ttl) = env::var("PROXY_CACHE_TTL_SECONDS") {
            config.behavior.cache_ttl_seconds = ttl.parse().map_err(|e| {
                ramparts_common::anyhow::anyhow!("Invalid cache_ttl_seconds value: {}", e)
            })?;
        }

        if let Ok(size) = env::var("PROXY_MAX_REQUEST_SIZE") {
            config.behavior.max_request_size = size.parse().map_err(|e| {
                ramparts_common::anyhow::anyhow!("Invalid max_request_size value: {}", e)
            })?;
        }
    ...
    pub fn validate(&self) -> Result<()> {
        if self.javelin.api_key.is_empty() {
            // Allow explicit test mode only if env flag is set
            let allow_test = env::var("RAMPARTS_ALLOW_TEST_MODE")
                .ok()
                .map(|v| v.eq_ignore_ascii_case("true"))
                .unwrap_or(false);
            if allow_test {
                ramparts_common::tracing::warn!(
                    "RAMPARTS_ALLOW_TEST_MODE=true and no JAVELIN_API_KEY set: running in test mode"
                );
            } else {
                return Err(ramparts_common::anyhow::anyhow!(
                    "JAVELIN_API_KEY is required (set RAMPARTS_ALLOW_TEST_MODE=true to bypass for test)"
                ));
            }
        }
