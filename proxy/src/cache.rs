...
impl ValidationCache {
    ...
    pub async fn get_or_compute<F, Fut>(
        ...
        request: &Value,
        compute_fn: F,
    ) -> Result<ValidationCacheEntry, ramparts_common::anyhow::Error>
    where
        F: FnOnce() -> Fut,
        Fut: std::future::Future<
            Output = Result<ValidationCacheEntry, ramparts_common::anyhow::Error>,
        >,
    {
        // First check cache
        ...
        if self.config.enable_deduplication {
            ...
            if let Some(sender) = self.pending_requests.get(&key) {
                tracing::debug!(
                    "Request deduplication: waiting for pending request {}",
                    &key[..16]
                );
...
impl ValidationCache {
    ...
    pub async fn batch_validate<F, Fut>(
        &self,
        batch: BatchValidationRequest,
        compute_fn: F,
    ) -> Result<BatchValidationResponse, ramparts_common::anyhow::Error>
    where
        F: FnOnce(Vec<(String, Value)>) -> Fut,
        Fut: std::future::Future<
            Output = Result<Vec<(String, ValidationCacheEntry)>, ramparts_common::anyhow::Error>,
        >,
    {
