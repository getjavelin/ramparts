...
impl JavelinClient {
        ...
        Self {
            ...
            base_url: base_url.unwrap_or_else(|| {
                std::env::var("JAVELIN_API_URL")
                    .unwrap_or_else(|_| "https://guard.model.javelin.live".to_string())
            }),
            client,
            cache,
        }
    ...
    /// Format MCP request content for Javelin Guard API
    fn format_request_for_guard(content: &Value) -> String {
        // Convert MCP request to natural language text for Guard API
                    ...
                    match name {
                        ...
                        "write_file" | "file_write" => {
                            if let Some(path) = args.get("path").and_then(|p| p.as_str()) {
                                let content_preview = args.get("content")
                                    ...
                                    .unwrap_or("data");
                                return format!("write file {} with content: {}", path, content_preview);
                            }
                        }
                        ...
                        "execute_command" | "run_command" | "shell_exec" => {
                            if let Some(command) = args.get("command").and_then(|c| c.as_str()) {
                                return format!("execute command: {}", command);
                            }
                        }
                        "database_query" | "sql_query" => {
                            if let Some(query) = args.get("query").and_then(|q| q.as_str()) {
                                return format!("run database query: {}", query);
                            }
                        }
                        "network_request" | "http_request" => {
                            if let Some(url) = args.get("url").and_then(|u| u.as_str()) {
                                return format!("make network request to {}", url);
                            }
                        }
                        _ => {
                            // Generic tool with arguments
                            return format!("use tool {} with arguments: {}", name,
                                serde_json::to_string(args).unwrap_or_default());
                        }
                    }
                }
                // Tool without arguments
                return format!("use tool {}", name);
            }
        }

        // Fallback: convert entire request to string
        format!("perform action: {}", serde_json::to_string(content).unwrap_or_default())
    }
