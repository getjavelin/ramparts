...
use rmcp::{
    ...
    transport::{io::stdio, sse_server::SseServer, streamable_http_server::{tower::StreamableHttpService, session::local::LocalSessionManager, StreamableHttpServerConfig}},
    ErrorData,
};
...
impl RampartsMcpServer {
    pub fn new() -> Self {
        ...
        Self { tool_router: Self::tool_router(), counter: Arc::new(Mutex::new(0)), core: Arc::new(core) }
    }
    ...
    async fn increment_counter(&self) -> Result<CallToolResult, ErrorData> {
        ...
        *guard += 1;
        Ok(CallToolResult::success(vec![Content::text(guard.to_string())]))
    }

    #[tool(name = "scan", description = "Scan an MCP server URL and return security findings as JSON")]
    async fn scan(&self, params: Parameters<ScanParams>) -> Result<CallToolResult, ErrorData> {
        ...
        if let Some(result) = resp.result {
            let json = serde_json::to_string(&result).map_err(|e| ErrorData::internal_error(e.to_string(), None))?;
            Ok(CallToolResult::success(vec![Content::text(json)]))
        } else {
            Err(ErrorData::invalid_request(resp.error.unwrap_or_else(|| "scan failed".to_string()), None))
        }
    }

    // New: scan-config tool
    #[tool(name = "scan-config", description = "Scan MCP servers from IDE configuration files and return results as JSON")]
    async fn scan_config(&self, params: Parameters<ScanConfigParams>) -> Result<CallToolResult, ErrorData> {
        let p = params.0;
        let mut builder = ScanConfigBuilder::new();
        if let Some(d) = p.detailed { builder = builder.detailed(d); }
        if let Some(fmt) = p.format { builder = builder.format(fmt); }
        if let Some(t) = p.timeout { builder = builder.timeout(t); }
        if let Some(ht) = p.http_timeout { builder = builder.http_timeout(ht); }
        if let Some(headers) = p.auth_headers { builder = builder.auth_headers(Some(headers)); }
        // Default to returning prompts (no LLM call) for MCP tool flow
        ...
        let results = scanner.scan_config_by_ide(options)
            .await
...
pub async fn run_streamable_http_server(host: &str, port: u16) -> Result<(), Box<dyn std::error::Error>> {
    use axum::Router;
    use axum::routing::any_service;
    use tower::ServiceBuilder;

    let bind: std::net::SocketAddr = format!("{}:{}", host, port).parse()?;
    let service: StreamableHttpService<RampartsMcpServer, LocalSessionManager> = StreamableHttpService::new(
